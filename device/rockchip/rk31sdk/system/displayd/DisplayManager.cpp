...
void DisplayManager::switchFramebuffer(int display, char* mode)
{
	char name[64];
	if(display == 0)
	...
	ALOGD("%s display %d mode %s", __FUNCTION__, display, mode);		
	int xres, yres;
	if(strcmp(mode, "1920x1080") == 0) {
		xres = 1920;
		yres = 1080;
	} else if (strcmp(mode, "1280x720") == 0) {
		xres = 1280;
		yres = 720;
	} else if (strcmp(mode, "720x576") == 0) {
		xres = 720;
		yres = 576;
	} else if (strcmp(mode, "720x480") == 0) {
		xres = 720;
		yres = 480;
	} else {
		ALOGE("%s not support mode %s", __FUNCTION__, mode);
		return;
	}
	ALOGD("%s new res is x %d y %d", __FUNCTION__, xres, yres);
